...
const NAV_ITEMS = [
  ...
  { label: "Tech LMS", href: "/tech", Icon: TechIcon },
  ...
  return (
    <aside className="w-64 border-r border-border flex flex-col bg-bg/50 backdrop-blur-xl h-screen sticky top-0">
      ...
      <div className="p-4 border-t border-border">
        <button
          onClick={lock}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-text-muted hover:text-red-500 hover:bg-red-500/5 transition-all group"
        >
          <LogOutIcon className="text-text-dim group-hover:text-red-500" />
          <span className="text-[10px] font-black tracking-widest uppercase">
            Lock System
          </span>
        </button>
...
function StudyIcon({ className }: { className?: string }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
  );
}
